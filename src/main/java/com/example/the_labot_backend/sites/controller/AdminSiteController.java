...
import com.example.the_labot_backend.sites.dto.SiteCreateRequest;
import com.example.the_labot_backend.sites.dto.SiteDetailResponse;
import com.example.the_labot_backend.sites.dto.SiteUpdateRequest;
...
public class AdminSiteController {
    ...
    public ResponseEntity<?> getSite(@PathVariable Long siteId) {
        ...
        SiteDetailResponse response = siteService.getSiteDetail(userId, siteId);
        return ResponseEntity.ok(Map.of(
                "status", 200,
                "message", "현장 상세 조회 성공",
                "data", response
    ...
    public ResponseEntity<?> updateSite(
        ...
        SiteDetailResponse response = siteService.updateSite(siteId, request);
